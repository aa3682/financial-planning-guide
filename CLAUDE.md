...
- Health plan out-of-pocket cap: normally taken from CMS's yearly premium-adjustment-percentage guidance (published for the following year, cms.gov/files/document/<year>-papi-parameters-guidance-<date>.pdf). Before using the guidance figure, check whether an HHS rule for that year superseded it; for 2026 the June 2025 rule at 90 FR 27074 did, and the rule's figure is the one cited.
...
- H2 sections in this order, headings only: "What this measures", "Before you start", first input section, second input section, "The result", "What the number does and does not tell you", "What to do with it", "For advisors" (Callout), then Related.
